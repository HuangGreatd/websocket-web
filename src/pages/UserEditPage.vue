        ...
        if (!reg_tel.test(<string>phone)) {
          showToast({message: '手机号格式错误'});
            ...
            if (flag) {
                ...
                lock.value = false
                if (res?.data.code === 0) {
                    Toast.success("短信发送成功，15分钟内有效")
                } else {
                    Toast.fail("短信发送失败," + res?.data.description ?? '')
                }
            }
        }
    }
    ...
    if (email === '') {
      Toast({message: '请先输入手机号'});
        ...
        if (flag) {
            ...
            lock.value = false
            if (res?.data.code === 0) {
                Toast.success("邮件发送成功，15分钟内有效")
            } else {
                Toast.fail("邮件发送失败," + res?.data.description ?? '')
            }
        }
